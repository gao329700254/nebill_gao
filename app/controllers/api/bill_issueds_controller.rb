class Api::BillIssuedsController < Api::ApiController
  ...
  def index
    @bills = Bill.issued_search_result(params[:expected_deposit_on_start], params[:expected_deposit_on_end]).where(status: 'issued').or(Bill.issued_search_result(params[:expected_deposit_on_start], params[:expected_deposit_on_end]).where(status: 'confirmed'))
