class Chatwork::FileUploadService < BaseService
  ...
  def execute
    if upload_user.blank?
      logger.info "user not exists chatwork_id: #{webhook_params[:from_account_id]}" and return
    end
...
private
  ...
  def create_expense!
    expense = upload_user.expenses.build(use_date: Date.today, default: default_item, amount: default_item.standard_amount || 100)
